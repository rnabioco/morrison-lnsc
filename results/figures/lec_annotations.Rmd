...
create_obj <- !file.exists(file.path(params$so_dir, "so_lec_annotations.qs"))
...
if (!create_obj) {
  so_lec_merge <- qread(file.path(params$so_dir, "so_lec_annotations.qs"))
...
if (create_obj) {
  so_lec_merge <- so_lec_merge %>%
    mutate_meta(
      mutate,
      new_type = ifelse(!!sym(clst) %in% top_marco, "Marco_LEC", type)
    )
  
  so_lec_merge %>%
    qsave(here(params$so_dir, "so_lec_annotations.qs"))
}
...
* Some of the BEC clusters seem to vary when clustering parameters are modified. This is because some BEC clusters are sometimes classified as "unassigned", not because they are sometimes classified as a LEC, i.e. the BECs are always classified as a BEC or unassigned
...
## Marco expression in CHIKV-high cells
