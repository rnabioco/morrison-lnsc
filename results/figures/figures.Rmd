...
Mxra8 expression is shown for CHIKV+/- cells for the 8 hpi timepoint. Mxra8 expression appears higher in CHIKV+ FRCs and BECs, suggesting a distinct mechanism of CHIKV entry. This is a result we discussed previously and could be included in a supplemental figure.
...
cnt_dat <- plt_dat@meta.data %>%
  mutate(
    nCount_RNA_CHIKV     = nCount_RNA + nCount_CHIKV,
    enr_nCount_RNA_CHIKV = enr_nCount_RNA + enr_nCount_CHIKV,
    treatment            = fct_relevel(treatment, treats)
  )

axis_labs <- c(
  nCount_RNA_CHIKV     = "Total counts",
  enr_nCount_RNA_CHIKV = "Total counts\nCHIKV-enriched libraries"
)

lab_dat <- cnt_dat %>%
  group_by(rep, treatment) %>%
  summarize(
    r = cor(nCount_RNA_CHIKV, enr_nCount_RNA_CHIKV, method = "pearson"),
    r = str_c("r = ", round(r, 3)),
...
go_comps  <- grep("^so_(8|24|merge)-up_lectype$", names(degs), value = TRUE)
...
A.  Dendrogram showing gene ontology modules. GO terms were identified for each cell type and filtered as described in Fig 5. Terms were clustered into 5 modules based on the pairwise overlap between terms.
