...
deg_args <- tribble(
  ...
  "so_24",       "lec_type", "treatment", "CHIKV",      "mock",      c(0.25, Inf),    # 24hpi-mock up/down
  ...
  "so_8_chikv",  "lec_type", "chikv_grp", "CHIKV-high", "CHIKV-low", c(0.15, Inf),    # 8hpi  CHIKV-high/low up
  "so_24_chikv", "lec_type", "chikv_grp", "CHIKV-high", "CHIKV-low", c(0.15, Inf),    # 24hpi CHIKV-high/low up
...
# Comparisons to use for GO analysis
...
go_ovlp <- set_names(c(3, 10), go_comps)

clst_args <- list(
  go_in = go[go_comps],
  ovlp  = go_ovlp[go_comps]
)
...
plt_args <- list(
  go_in    = go[go_comps],
  clst_dat = clusters[go_comps],
  clrs     = go_clrs[go_comps],
  ovlp     = go_ovlp[go_comps],
  clsts    = list(1, 1:2)
)
...
```{r "GO CLUSTER TERM SIMILARITY", eval = FALSE}
...
plt_args <- list(
  go_in   = go[go_comps],
  degs_in = degs[go_comps],
  heat    = go_heat[go_comps],
  clrs    = go_clrs[go_comps],
  genes   = list(
    c("Irf7", "Bst2", "Zbp1"),
...
bot <- plot_grid(
  ...
  go_examples$`so_merge-up_lectype`,
...
plt_args %>%
  pwalk(~ {
    ...
    ttl <- str_extract(args$ttl, "[0-9a-zA-Z]+-[a-zA-Z]+")
    ttl <- str_replace(ttl, "^merge", "24hpi-8hpi")
    ttl <- str_replace(ttl, "^8", "8hpi-mock")
    
    cat("\n\n#### ", ttl, "\n\n")
    
    plt <- create_go_summary_plots(
      go_in      = args$go_in,
      degs_in    = args$degs_in,
      n_overlap  = args$ovlp,
      n_clusters = 5,
