...
go_comps  <- grep("^so_(8|24|merge)(_chikv|)-(up|down)_lectype$", names(degs), value = TRUE)
...
plt_args %>%
  pwalk(~ {
    ...
    ttl <- str_remove(args$ttl, "^so_")
    ttl <- str_remove(ttl, "_[a-zA-Z]+$")
    ttl <- str_replace(ttl, "^merge", "24-8")
