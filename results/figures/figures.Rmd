...
# Hypergeometric test
# values are arranged by column
.calc_fisher <- function(...) {
  res <- c(...) %>%
    matrix(nrow = 2) %>%
    as.data.frame() %>%
    fisher.test() %>%
...
frac_dat <- frac_dat %>%
  ...
  mutate(
    type_tot = type_tot - n,
    n_tot    = n_tot - n,
    tot      = tot - n_tot - type_tot,
    p_val    = .calc_fisher(n, type_tot, n_tot, tot)
  ) %>%
  ungroup() %>%
  arrange(p_val) %>%
...
frac_8 <- frac_dat %>%
  ...
  geom_text(
    # aes(y = frac + (max(frac) * 0.05), label = p_lab),
    aes(label = p_lab),
