...
```{r "% CHIKV umap", fig.width = 10, fig.height = 10}
# Format plot data
clmn <- c(enr_pct_CHIKV = "CHIKV score")

dat <- so_8@meta.data %>%
  arrange(!!sym(names(clmn)))
...
# Create UMAP
pseudo <- dat[[names(clmn)]]
pseudo <- min(pseudo[pseudo > 0]) / 2

pct_chikv_umap <- dat %>%
  ggplot(aes(UMAP_1, UMAP_2, fill = !!sym(names(clmn)) + pseudo)) +
  geom_point_trace(
    color = "grey",
  ...
  guides(fill = guide_colorbar(title.position = "top", ticks = FALSE, title = clmn)) +
  umap_theme +
  theme(
    aspect.ratio         = 0.9,
    legend.position      = "top",
...
```{r "CHIKV-high umap", fig.width = 10, fig.height = 10}
# Format plot data
...
```{r "% CHIKV boxes", fig.width = 10, fig.height = 5}
# Calculate p-values
# only calculate p-values for cell types included fraction bargraphs
bx_dat <- dat %>%
  filter(chikv_grp == chikv_grps[2])
...
p_dat <- bx_dat %>%
  ...
  map_dfr(~ {
    ...
    typ_dat <- d[[.x]][[names(clmn)]]
    oth_dat <- d[["other"]][[names(clmn)]]
...
count_bxs <- bx_dat %>%
  ...
  ggplot(aes(lec_type, !!sym(names(clmn)), fill = lec_type, color = lec_type)) +
  geom_boxplot(
...
The CHIKV score shown in figures 1A and C is calculated by dividing CHIKV counts from the CHIKV-capture library by total mouse counts from the CHIKV-capture library for each cell. For this calculation we are assuming that mouse counts from the capture library reflect relative mouse counts from the original libraries.

There is a strong correlation between counts from the capture and original libraries, this validates the use of mouse counts from the capture libraries as a normalization factor.
