...
go_comps  <- grep("^so_(8|24|merge)-(up|down)_lectype$", names(degs), value = TRUE)
...
# Determine which comparisons are for upregulated genes
go_upreg <- set_names(grepl("-up", go_comps), go_comps)

plt_args <- list(
  go_in      = go[go_comps],
  degs_in    = degs[go_comps],
  ovlp       = go_ovlp[go_comps],
  only_upreg = go_upreg[go_comps],
  ttl        = go_comps
)
...
plt_args %>%
  pwalk(~ {
    ...
    degs_type <- ifelse(args$only_upreg, "up", "down")
    
    degs_type <- c("8", "merge") %>%
      str_c("so_", ., "-", degs_type, "_lectype")
    
    all_degs_data = list(
      "8hpi"  = degs[[degs_type[1]]],
      "24hpi" = degs[[degs_type[2]]]
    )
    ...
    plt <- create_go_summary_plots(
      ...
      only_show_upreg = args$only_upreg,
      exclude_cell_types = exclude_cell_types
    )
