...
clusters <- go %>%
  imap(~ {
    ...
    k    <- ifelse(grepl("^so_8-", .y), 2, 3)
    
    get_clusters(.x, k = k, n_overlap = ovlp)
  }) %>%
  discard(is.null)
...
create_gene_plots <- function(df_in, gns, deg_data = degs,
                              clrs = c(`1` = "orange"), cell_types = NULL,
                              lvls = c("mock", "8hpi", "24hpi"), n_cells = 10) {
  
  genes <- names(gns)
  
  df_in <- df_in %>%
    dplyr::filter(name %in% genes) %>%
...
gene_dat <- list("8hpi" = so_8, "24hpi" = so_24) %>%
  imap_dfr(~ {
    .x %>%
      FetchData(c("subtype", "orig.ident", "treatment", "rep", names(gns))) %>%
      as_tibble(rownames = ".cell_id") %>%
      mutate(tm = .y) %>%
...
deg_data <- ln_trts %>%
  imap_dfr(~ {
    degs[[.x]] %>%
      dplyr::filter(gene %in% names(gns)) %>%
...
```{r "GO figures", fig.width = 10.5, fig.height = 8.5}
