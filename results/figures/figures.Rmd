...
frac_dat <- frac_dat %>%
  group_by(lec_type) %>%
  mutate(p_val = .calc_fisher(type_n, type_tot, n, tot - n)) %>%
  ungroup() %>%
  arrange(p_val) %>%
...
# Create bargraphs
# Only include cell types with >40 total cells, >3 CHIKV+ cells
frac_dat <- frac_dat %>%
...
# 84/191	632/15076	6.588999771563824e-65
# 
# data.frame(
#   up    = c(84, 191 - 84),
#   other = c(632, 15076 - 632),
#   row.names = c("in_term", "out_term")
# ) %>%
#   t() %>%
#   # chisq.test() %>%
#   fisher.test(alternative = "t") %>%
#   tidy()
```
...
bx_lvls <- bx_dat %>%
  group_by(lec_type) %>%
  summarize(
    med = median(CHIKV_sgRNA_fc_raw),
    q3  = quantile(CHIKV_sgRNA_fc_raw, 0.75),
    max = max(CHIKV_sgRNA_fc_raw),
    .groups = "drop"
  ) %>%
...
bx_p_vals <- bx_dat %>%
  calc_p_vals(
    data_column = "CHIKV_sgRNA_fc_raw",
    type_column = "lec_type"
  )

p_file <- here("results/tables/figS2_sgRNA_pval.tsv")
