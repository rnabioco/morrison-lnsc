...
create_go_heatmap <- function(df_in, go_in, n_clusters = 2, n_terms = 5,
  ...
  res <- dat %>%
    ggplot(aes(Cluster, Description, fill = as.character(cluster), alpha = GeneRatio)) +
    geom_tile() +
    geom_point(
    ...
    scale_alpha_continuous(range = c(0, 1), n.breaks = 4) +
...
Differentially expressed genes were identified for each cell type for mock vs 8 hpi and 8 hpi vs 24 hpi. Genes were considered upregulated if the log2 fold change was >0.15 for 8 hpi and >0.25 for 24 hpi for all replicates and the maximum p-value for all replicates was <0.05.

Gene ontology terms (biological process) were then identified for upregulated genes for each cell type. Terms were filtered to only include those with an adjusted p-value <0.05 and at least 3 or 10 upregulated genes overlapping the term for the 8 hpi and 24 hpi timepoints, respectively. Terms identified for all cell types were combined together and clustered into three modules based on similarity.
...
C.  The 8 hpi timeopoint shows upregulation of genes associated with an interferon-beta response. The fraction of upregulated genes overlapping terms from the primary gene ontology module is shown for the 8 hpi timepoint.
D.  Top upregulated genes identified for the terms shown in A are plotted. Points show the median expression for the mock (m), 8 hpi (8h), and 24 hpi (24h) samples, grey bars show the interquartile range. A solid line indicates that the gene is significantly upregulated between the samples.
E.  The fraction of upregulated genes overlapping terms from the primary gene ontology modules is shown for the 24 hpi timepoint, as described in C.
...
ttls <- as_labeller(c(`1` = "response to interferon-beta", `2` = "response to cytokine stimulus"))
...
gns <- go$`so_8-up`@compareClusterResult %>%
  as_tibble() %>%
  right_join(go_heat$`so_8-up`$terms, by = "Description") %>%
  mutate(geneID = str_split(geneID, "/")) %>%
  unnest(geneID) %>%
...
gns <- go$`so_merge-up`@compareClusterResult %>%
  as_tibble() %>%
  right_join(go_heat$`so_merge-up`$terms, by = "Description") %>%
  mutate(geneID = str_split(geneID, "/")) %>%
  unnest(geneID) %>%
  group_by(geneID, cluster) %>%
  summarize(n = n_distinct(Cluster), .groups = "drop") %>%
...
```{r "GO figures", fig.width = 12, fig.height = 9}
...
plot_grid(top, bot, ncol = 1, rel_heights = c(0.68, 1))
...
create_go_summary_figures <- function(go_in, sample_name, n_clsts = 5,
  ...
  tr <- ggtree(clsts$object, linewidth = 1.5)
  ...
  gns <- go_typ@compareClusterResult %>%
    as_tibble() %>%
  ...
  gn_plts <- create_gene_plots(
    ...
    theme(plot.margin = margin(6, 40, 6, 40, unit = "pt"))
