...
# # To plot density for hypergeometric distribution for fLECs
# dhyper(
#   x = 1:23,  # number of white balls drawn
#   k = 74,    # number of balls drawn
#   m = 267,   # number of white balls in urn
#   n = 3286   # number of black balls in urn
# )
# 
# # As a test, calculate p-value for identitified GO term
# # file  ID  GeneRatio BgRatio pvalue  p.adjust
# # merge_lectype_chikv-up  GO:0045087  84/191  632/15076 6.588999771563824e-65 1.7875956380252655e-61
# 
# .calc_fisher(
#   x = 84,
#   k = 191,
...
top <- plot_grid(
  chikv_score_umap, chikv_typs_umap,
...
### Marco/Mxra8 expression

Mxra8 expression is shown for CHIKV+/- cells for the 8 hpi timepoint. Mxra8 expression appears to be higher in CHIKV+ FRCs and BECs, suggesting a distinct mechanism of CHIKV entry. This is a result we have discussed previously, and could be included in a supplemental figure.

```{r, fig.width = 12, fig.height = 5}
mxra8_bxs <- lec_type_cols[c("Marco_LEC", "FRC", "BEC")] %>%
  imap(~ {
    plt_dat %>%
      create_gene_boxplot(
        GENE = "Mxra8", TYPE = .y,
        clr = .x, lvls = plt_lvls, p_size = txt_pt1
      )
  })

plot_grid(plotlist = mxra8_bxs, nrow = 1)
```

Marco and Mxra8 expression is shown for CHIKV-high/low cells for the 24 hpi timepoint. Using the updated cell type annotations, we still observe increased Marco expression for CHIKV-high Marco LECs. There is no difference in Mxra8 expression for CHIKV-high FRCs and BECs at 24 hpi.

```{r, fig.width = 12, fig.height = 5}
plts <- lec_type_cols[c("FRC", "BEC")] %>%
  imap(~ {
    so_24 %>%
      create_gene_boxplot(
        GENE = "Mxra8", TYPE = .y,
        clr = .x, p_size = txt_pt1
      )
  })

so_24 %>%
  create_gene_boxplot(
    GENE = "Marco", TYPE = "Marco_LEC",
    clr = lec_type_cols[["Marco_LEC"]]
  ) %>%
  list() %>%
  append(plts) %>%
...
The CHIKV score shown in figures 1A and C is calculated by dividing CHIKV counts from the CHIKV-capture library by total counts (CHIKV + mouse) from the CHIKV-capture library for each cell. For this calculation we are assuming that mouse counts from the capture library reflect relative mouse counts from the original libraries (i.e. the relationship between CHIKV counts and mouse counts has not been affected by CHIKV enrichment).

There is a strong correlation between mouse counts from the capture libraries and original libraries, this validates the use of mouse counts from the capture libraries as a normalization factor.

```{r, fig.width = 6, fig.height = 6}
cnt_dat <- plt_dat@meta.data %>%
  mutate(treatment = fct_relevel(treatment, treats))

axis_labs <- c(
  nCount_RNA     = "Mouse counts",
  enr_nCount_RNA = "Mouse counts\nCHIKV-enriched libraries"
)
...
cnt_dat %>%
  ggplot(aes(!!sym(names(axis_labs)[1]), !!sym(names(axis_labs)[2]))) +
  geom_point() +
...
frac_24 <- frac_dat %>%
  ...
  theme(
    axis.text.y     = element_text(size = txt_pt1),
    axis.text.x     = element_text(size = txt_pt2, angle = 45, hjust = 1),
    axis.title.x    = element_blank(),
    legend.position = "none",
    aspect.ratio    = 0.25,
...
B.  UMAP projection shows CHIKV sgRNA ratio (sgRNA counts / 5' counts) for the 24 hpi timepoint. A pseudo count of 1 was added to the sgRNA and 5' counts to elminate division by zero.
C.  The fraction of cells identified as CHIKV-high is shown for each biological replicate for cell types with >40 total cells and >3 CHIKV-high cells for the 24 hpi timepoint. Labels show the number of CHIKV-high cells/total cells. P values were calculated using a one-sided hypergeometric test with Bonferroni correction to determine whether the frequency of CHIKV-high cells was greater than expected for each cell type. Only adjusted p values <0.05 are shown.
...
```{r "fig S2", fig.width = 16, fig.height = 14}
top <- plot_grid(
...
```{r "fig S3 cell type heatmaps OLD", fig.width = 8, fig.height = 5, eval = FALSE}
# Format heatmap data
h_dat <- so_lec %>%
  SplitObject("tm") %>%
  imap(~ {
...
plot_grid(
  type_fig, NULL, heats_fig, NULL, type_boxes,
...
This is an alternate version of Figure S3 that includes plots showing Mxra8 expression for CHIKV+ FRCs and BECs for the 8 hpi timepoint.

```{r "fig S3 v2", fig.width = 12, fig.height = 20}
# Add Mxra8 plots to figure
d <- mxra8_bxs[c("FRC", "BEC")] %>%
  map(~ {
    .x +
      theme(
        axis.title.y = element_text(size = txt_pt2),
        axis.text.x  = element_text(size = txt_pt1),
        axis.text.y  = element_text(size = txt_pt1),
...
plot_grid(
  type_fig, NULL, heats_fig_v2, NULL, type_boxes,
  rel_heights = c(0.7, 0.04, 0.4, 0.04, 1.2),
  labels      = c("", "", "", "", "E"),
  label_size  = ttl_pt2 * 1.5,
  ncol        = 1
...
deg_args <- tribble(
  ...
  "so_8",        "lec_type", "treatment", "CHIKV",      "mock",      c(0.15, Inf),
  "so_8",        "lec_type", "treatment", "CHIKV",      "mock",      c(-Inf, -0.15),
  "so_24",       "lec_type", "treatment", "CHIKV",      "mock",      c(0.25, Inf),
  "so_24",       "lec_type", "treatment", "CHIKV",      "mock",      c(-Inf, -0.25),
  "so_merge",    "lec_type", "treatment", "24hpi",      "8hpi",      c(0.25, Inf),
  "so_merge",    "lec_type", "treatment", "24hpi",      "8hpi",      c(-Inf, -0.25),
  "so_8_chikv",  "lec_type", "chikv_grp", "CHIKV-high", "CHIKV-low", c(0.15, Inf),
  "so_24_chikv", "lec_type", "chikv_grp", "CHIKV-high", "CHIKV-low", c(0.15, Inf),
  
  "so_8",        "cell_type_2", "treatment", "CHIKV",   "mock",      c(0.15, Inf),
  "so_8",        "cell_type_2", "treatment", "CHIKV",   "mock",      c(-Inf, -0.15),
  "so_merge",    "cell_type_2", "treatment", "24hpi",   "8hpi",      c(0.25, Inf),
...
go_degs <- degs[grepl("^so_(8|merge)-up_lectype$", names(degs))] %>%
  map(~ {
    .x %>%
      arrange(max_pval) %>%
      split(.$lec_type) %>%
...
# cluster_terms <- clusters %>%
#   imap(~ {
#     res <- .x$clusters %>%
#       filter(n > 1) %>%
#       split(.$cluster) %>%
#       map(dplyr::select, Description, ID)
# 
#     names(res) <- str_c(.y, "-", names(res))
# 
#     res
#   }) %>%
#   flatten()
# 
# term_sim <- cluster_terms %>%
#   imap_dfr(~ {
#     comp  <- .y
#     terms <- .x$ID
# 
...
#                 term1 = id,
#                 term2 = .x,
#                 sim   = goSim(id, .x, go_sim_data)
#               )
#             })
#         })
#       ) %>%
#       unnest(sim) %>%
#       group_by(comp, Description, ID) %>%
#       summarize(
#         n_terms  = n(),
#         mean_sim = mean(sim),
#         med_sim  = median(sim),
...
#           mutate(comparison = comp, cluster = .y)
#       })
#   })
# 
# # Identify similar clusters between comparisons
...
obj <- "so_8-up_lectype"
genes_to_plot <- c("Irf7", "Bst2", "Zbp1")
...
other_examples <- create_gene_plots(
  exp_data   = list("8hpi" = so_8, "24hpi" = so_merge),
  deg_data   = list("8hpi" = degs_8_merged, "24hpi" = degs_24_merged),
  gns        = genes_to_plot,
  clrs       = clrs_24,
  ...
  guides(linetype = guide_legend(
    title = "Differentially expressed", title.position = "top"
  )) +
...
```{r "expanded GO figures", fig.width = 16, fig.height = 19, results = "asis"}
...
create_go_summary_figures <- function(go_in, sample_name, n_clsts = 5,
                                      ...
                                      qval = 0.2, go_obj_sfx = "-up_lectype") {
  ...
  gn_plts <- create_gene_plots(
    exp_data   = list("8hpi" = so_8, "24hpi" = so_merge),
    deg_data   = list(
      "8hpi"  = degs[[str_c("so_8", go_obj_sfx)]],
      "24hpi" = degs[[str_c("so_merge", go_obj_sfx)]]
    ),
    gns        = gns,
    cell_types = 8,
    clrs       = clst_clrs,
...
```{r "GO CLUSTER GENE OVERLAP", eval = FALSE}
# Pull upregulated genes for each GO cluster for each timepoint
clst_terms <- clusters %>%
  map(~ {
    .x$clusters %>%
      split(.$cluster) %>%
...
# Calculate Jaccard index for each comparison
.calc_jaccard <- function(x, y) {
  length(intersect(x, y)) / length(union(x, y))
}

cluster_overlap <- clst_gns$`so_8-up_lectype` %>%
  imap_dfr(~ {
    gns1  <- .x
    clst1 <- .y
    
    clst_gns$`so_merge-up_lectype` %>%
      imap_dfr(~ {
        tibble(
          `8hpi_cluster`  = clst1,
          `24hpi_cluster` = .y,
          jaccard         = .calc_jaccard(gns1, .x)
        )
      })
  })
...
# Create heatmap
cluster_overlap %>%
  ggplot(aes(`8hpi_cluster`, `24hpi_cluster`, fill = jaccard)) +
  geom_tile() +
  base_theme
```
