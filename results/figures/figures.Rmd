...
marco_bxs_2 <- plt_dat %>%
  create_gene_boxplot("Marco", "Marco_LEC", clr = "#D7301F")
...
bot <- plot_grid(
  ...
  nrow   = 1,
  rel_widths = c(1.3, 0.05, 0.36, 0.05, 0.36, 0.25)
)
...
typ_bx_dat <- list("8hpi" = so_8, "24hpi" = so_24) %>%
  imap_dfr(~ {
    if (identical(.y, "8hpi")) {
      .x <- .x %>%
        mutate_meta(mutate, frc_type = frc_subtype)
    }
...
For this version, correlations are calculated using the combined data for all samples

```{r "fig S1 cell type v2", fig.width = 12, fig.height = 18}
plot_grid(
  type_heats_v2, NULL, type_boxes,
  rel_heights = c(0.7, 0.04, 1),
  label_size  = ttl_pt2 * 1.5,
  labels      = c("A", "B"),
  ncol        = 1
...
UMAP projections show cell type (A) and endothelial subset (B) annotations for integrated data.
...
create_type_umaps <- function(so_in, data_col, x = "hUMAP_1", y = "hUMAP_2", clrs = type_cols,
                              ttl = NULL, n_lab = TRUE, facet_plot = c("tm", "treatment"), ...) {
  res <- so_in %>%
    mutate_meta(~ {
      .x %>%
        group_by(tm, treatment) %>%
...
ttls <- as_labeller(c(`1` = "Innate Immune Response", `3` = "Inflammatory Response"))
...
gns <- go[[obj]]@compareClusterResult %>%
  ...
  filter(geneID %in% c("Bst2", "Zbp1", "Irf7"))
...
bx_plt_dat <- typ_bx_dat %>%
  mutate(
    ...
    new_frc_type  = ifelse(cell_type %in% c(fib_cell_types, "FRC"), frc_type, "other")
  )
...
mrc_bars <- abun_dat %>%
  ...
  plot_frequency(
    "new_frc_type", "orig.ident", "treatment",
    p_method = "t",
...
# so_24 %>%
#   plot_violin(
#     "Cxcl13", "frc_type", "treatment",
#     method = "boxplot"
#   )
# 
# so_24 %>%
#   plot_scatter(
#     "frc_type", group_col = "treatment",
#     size = 0.5
#   )
# 
# so_24 %>%
...
#   filter(tm == "8hpi") %>%
#   plot_frequency(
#     "new_frc_type", "orig.ident", stack = TRUE,
#     plot_colors = subtype_cols
#   ) 
# 
# bx_plt_dat %>%
#   filter(tm == "24hpi", new_frc_type != "other") %>%
#   plot_frequency(
#     "new_frc_type", "treatment",
#     p_label = "none"
#   )
```
