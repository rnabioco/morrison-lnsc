...
# so_merge %>%
#   qsave("~/Dropbox/Ryan/Projects/morrison-lnsc/results/sobjs/so_merge_int.qs")
```
...
```{r "refine consensus annotations", eval = create_obj}
...
so_lec <- so_lec %>%
  mutate_meta(mutate, lec_subtype = top_type) %>%
  classify_markers(
    feats    = "Marco",
    filt     = Marco > 0.5,
    type     = "Marco_LEC",
...
if (!file.exists(so_24_dir) || !file.exists(so_8_dir) && create_obj) {
  ...
  so_8 <- so_8 %>%
    mutate_meta(
      mutate,
      frc_type = if_else(fib_subtype == "other", cell_type, fib_subtype),
      cell_type = case_when(
        frc_type == "PvC"             ~ frc_type,
        cell_type %in% fib_cell_types ~ "FRC",
        TRUE                          ~ cell_type
  ...
  # add cluster to so_frc, re-cluster, re-annotate
  so_8 <- so_8 %>%
    mutate_meta(
      mutate,
...
so_merge <- so_merge %>%
  AddMetaData(FetchData(so_frc, "type"), col.name = "frc_type") %>%
  mutate_meta(
    mutate,
    new_cell_type = ifelse(frc_type == "PvC" & !is.na(frc_type), frc_type, type)
  )
