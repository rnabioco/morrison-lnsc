...
```{r "TOP CELLCHAT PATHWAYS", eval = FALSE}
...
#       geom_tile() +
#       scale_fill_gradient2(low = "blue", mid = "white", high = "red") +
#       facet_wrap(~ cluster, scales = "free", strip.position = "left") +
#       djvdj_theme() +
#       theme(
#         axis.text.y  = element_blank(),
#         axis.ticks.y = element_blank(),
...
go_bars <- go_res %>%
  ...
  scale_fill_manual(values = ito_cols) +
...
clst_gns <- clsts %>%
  ...
  group_by(cluster) %>%
  filter(cell_type %in% unique(cell_type[fc > log2(1.25) & p < 0.05])) %>%
  ungroup() %>%
  
  group_by(cluster, gene) %>%
  summarize(fc = mean(avg_log2FC), p = mean(max_pval), .groups = "drop") %>%
  arrange(cluster, desc(fc)) %>%
  # group_by(cluster) %>%
  # slice_max(fc, n = 10) %>%
  # ungroup() %>%
  mutate(cluster = fct_relevel(cluster, names(clst_lvls)))

# Select genes to plot
gns <- clst_gns %>%
  filter(cluster == names(clst_lvls)[1]) %>%
  filter(grepl("Isg|Ifi|Irf|Ccl|Cxc", gene)) %>%
  head(6) %>%
  pull(gene)
...
gns <- c("Irf7", "Ccl2", "Cxcl9", "Tlr3", "Vcam1", "Oas3")
...
# clst_gns %>%
#   filter(cluster %in% plt_clsts[1:4]) %>%
#   split(.$cluster) %>%
...
#     
#     so_24 %>%
#       FetchData(c("treatment", "lec_type", gns)) %>%
#       
#       pivot_longer(all_of(gns)) %>%
#       mutate(treatment = fct_relevel(treatment, treats)) %>%
#       
...
# mutate(
#   pos_typs = str_c(unique(cell_type[fc > log2(1.25) & p < 0.05]), collapse = "-")
# )
