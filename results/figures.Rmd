...
create_cor_heatmap <- function(df_in, ttl, lec_labs = NULL) {
...
```{r "fig S1 v1", fig.width = 15, fig.height = 14}
left <- plot_grid(
  sgRNA_boxes, scatter,
  labels = c("B", "C"), label_size = ttl_pt2 * 1.5,
  align = "v", axis = "rl",
  rel_heights = c(0.7, 1),
  ncol = 1
)
...
top <- plot_grid(
  sgRNA_umap, left,
  labels = c("A", ""), label_size = ttl_pt2 * 1.5,
  rel_widths = c(0.8, 1),
  nrow = 1
)
...
plot_grid(
  top, type_boxes,
  labels = c("", "D"), label_size = ttl_pt2 * 1.5,
  align = "v", axis = "rl",
  ncol = 1
)
```
...
```{r "H FIGURE", fig.height = 10, fig.width = 10}
