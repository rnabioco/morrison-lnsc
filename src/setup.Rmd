...
get_go <- function(gns, so_in, type_clmn = "subtype", max_term_size = 750,
  ...
  bkgd <- gns %>%
    imap(~ {
      ...
      # if gns is not named, all cells in so_in will be used
      if (!is.null(so_in)) {
...
get_clusters <- function(go_in, k = 3, pval = 0.05, qval = 0.2, n_overlap = 3,
  ...
  top_terms <- go_in@compareClusterResult %>%
    separate(
      GeneRatio,
      into = c("n_ovlp", "tot_genes"),   # split gene ratios 
      sep = "/", convert = TRUE
    ) %>%
    separate(
      BgRatio,
      into = c("n_bg_ovlp", "tot_bg_genes"),
      sep = "/", convert = TRUE
    ) %>%
    mutate(
      GeneRatio  = n_ovlp / tot_genes,   # calculate enrichment scores
