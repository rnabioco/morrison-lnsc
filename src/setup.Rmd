...
create_go_heatmap <- function(df_in, go_in, n_clusters = 2, n_terms = 5,
  ...
  terms <- df_in %>%
    ...
    mutate(shared = n_cell_types > 1) %>%
    arrange(cluster, desc(n_cell_types), desc(enrichment)) %>%
    group_by(cluster) %>%
    dplyr::slice(n_seq) %>%
    ungroup()
  
  # Max number of plotted terms, used to set aspect ratio for plot
  n_terms <- terms %>%
    group_by(cluster) %>%
  ...
  # Data for heatmap
  # filter cell types (Cluster column) here
  cluster_ids <- df_in %>%
    dplyr::select(Description, cluster)
  
  agg_lab <- "Overall Enrichment"
  lvls    <- rev(terms$Description)
  ...
  # filter cell types based on total number of unique DEGs overlapping
  # plotted terms
  agg_dat <- dat
  ...
  agg_dat <- agg_dat %>%
    ...
    summarize(
      ...
      tot_bg_ovlp = n_distinct(reduce(bgID, c)),
      .groups     = "drop"
    ) %>%
    mutate(
...
#' provided genes are differentially expressed and overlap a GO term, and
#' return the GO cluster assignment for the gene (cluster with at least one term
#' the gene overlaps).
#' 
